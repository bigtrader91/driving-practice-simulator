...
describe('missionForTrainingAttempt', () => {
  it('좌우 시도에 맞게 시작 차로와 목표 차로를 서로 반대로 배치한다', () => {
    ...
    const left = missionForTrainingAttempt(laneChangeMission, 'left', 1);
    const right = missionForTrainingAttempt(laneChangeMission, 'right', 2);
    ...
    expect([left.visualVariant, right.visualVariant]).toEqual([1, 2]);
    expect([laneChangeMission.startPos[0], laneChangeMission.targetArea?.x]).toEqual([7, -7]);
    expect(laneChangeMission.visualVariant).toBeUndefined();
  });
});
